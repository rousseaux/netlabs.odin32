...
PciRegister PCI_IO_BASE_ADDRESS = { 0x18, 4, READ_WRITE };
PciRegister PCI_SUBVENDOR_ID    = { 0x2C, 4, READ_ONLY };
PciRegister PCI_SUBSYSTEM_ID    = { 0x2E, 4, READ_ONLY };
PciRegister PCI_ROM_BASE_ADDRESS= { 0x30, 4, READ_WRITE };
PciRegister PCI_CAP_PTR         = { 0x34, 4, READ_WRITE };
...
PciRegister PCI_FAB_ID          = { 0x40, 1, READ_ONLY };
PciRegister PCI_CONFIG_STATUS   = { 0x4C, 4, READ_WRITE };
PciRegister PCI_CONFIG_SCRATCH  = { 0x50, 4, READ_WRITE };
PciRegister PCI_AGP_CAP_ID      = { 0x54, 4, READ_ONLY };
PciRegister PCI_AGP_STATUS      = { 0x58, 4, READ_ONLY };
PciRegister PCI_AGP_CMD         = { 0x5C, 4, READ_WRITE };
PciRegister PCI_ACPI_CAP_ID     = { 0x60, 4, READ_ONLY };
PciRegister PCI_CNTRL_STATUS    = { 0x64, 4, READ_WRITE };
...
#ifdef __linux__
  if (hasDev3DfxLinux()) return pciOpenMyOS();
#endif
#ifdef __OS2__
  if (hasDev3DfxOS2()) return pciOpenMyOS();
#endif
...
#ifdef __OS2__  
  if (hasDev3DfxOS2()) {
    *data = pciFetchRegisterOS2(reg.regAddress, reg.sizeInBytes,
				  device_bus_func_number);
    return FXTRUE;
  }
#endif
...
#ifdef __OS2__
  if (hasDev3DfxOS2()) {
    return pciUpdateRegisterOS2(reg.regAddress, *data, reg.sizeInBytes,
				  device_bus_func_number);
  }
#endif
